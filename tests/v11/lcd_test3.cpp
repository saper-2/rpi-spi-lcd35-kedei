// ************ SPI TEST APP **************
// Test 3 - SPI transmission moddified to use single SPI_IOC_MESSAGE
// ----------------------------------------
...
void lcd_cmd(int *spih, uint16_t cmd) {
	...
	buff[1] =  cmd>>8;
	buff[2] = cmd&0x00ff;
	buff[3] = 0x11; // 0x11 - CMD_BE const from ili9341.c (BE is short form "before")
	r = spi_transmit(spih, &buff[0], 4, LCD_SPI_SPEED, LCD_SPI_BITS_PER_WORD);
	if (r < 0) {		
	...
	buff[1] =  cmd>>8;
	buff[2] = cmd&0x00ff;
	buff[3] = 0x1B; // 0x1B - CMD_AF const from ili9341.c (AF is short form "after")
	r = spi_transmit(spih, &buff[0], 4, LCD_SPI_SPEED, LCD_SPI_BITS_PER_WORD);
	if (r < 0) {		
